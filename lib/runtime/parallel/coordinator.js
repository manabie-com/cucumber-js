...
class Coordinator {
    ...
    startWorker(id, total) {
        const workerProcess = child_process_1.fork(runWorkerPath, [], {
            cwd: this.cwd,
            env: Object.assign(Object.assign({}, process.env), { CUCUMBER_PARALLEL: 'true', CUCUMBER_TOTAL_WORKERS: total.toString(), CUCUMBER_WORKER_ID: id }),
    ...
    shouldCauseFailure(status) {
        return (['AMBIGUOUS', 'FAILED', 'UNDEFINED'].includes(status) ||
